return {
	on_click = function(player, object)
		...
		local gui = player.gui.center.uc["uc-max-combinator"]
		for i = 1,5 do
	...
	on_key = function(player, object)
		if not (player.gui.center["uc"]) then
			...
			local layout = uc.add{type = "table", name = "uc-max-combinator", column_count = 6}
			layout.add{type = "label", caption = "Filter: (?)", tooltip = {"uc-max-combinator.filter"}}
			for i = 1,5 do
